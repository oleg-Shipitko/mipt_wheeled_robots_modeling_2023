...
sudo docker run -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix --name hw3 -it homework  

Этой командой мы создаём контейнер с именем hw3 из собранного ранее образа с именем homework

**Внимание!** Если при запуске графических приложений из контейнера вы получаете ошибку, выполните команду xhost +local: на своей хост-машине.

*** Также можно попробовать DISPLAY=unix$DISPLAY

Для запуска дополнительного терминала в контейнер, можно выяснить CONTAINER ID, выполнив команду docker ps
и найдя в выводе CONTAINER ID, необходимого контейнера. После этого необходимо выполнить команду:
...
cd /homework/homework3/CarND-PID-Control-Project
...
Среда Visual Studio Code может быть запущена командой:
 ...
 code <директория с исходным кодом> --user-data-dir=/vscode
